...
from toolbox.models.teacherb4_studentb0_model17_2Net.segformer.mix_transformer import mit_b2
import math
...
class DCNet_S(nn.Module):
    def __init__(self, channel=64):
        super(DCNet_S, self).__init__()
        self.rgb = mit_b2()
    ...
    def forward(self, x, x_depth):
        ...
        x1_depth = self.rgb.forward_featuresOne(x_depth)
        x1_1 = self.dea1(x1, x1_depth)
        x2=x1+x1_1
        x2_depth=x1_depth+x1_1
        x2 = self.rgb.forward_featuresTwo(x2)
        x2_depth = self.rgb.forward_featuresTwo(x2_depth)
        x2_1 = self.dea2(x2, x2_depth)
        x3 = x2 + x2_1
        x3_depth = x2_depth + x2_1
        x3 = self.rgb.forward_featuresThree(x3)
        x3_depth = self.rgb.forward_featuresThree(x3_depth)
        x3_1 = self.dea3(x3, x3_depth)
        x4 = x3 + x3_1
        x4_depth = x3_depth + x3_1
        x4 = self.rgb.forward_featuresFour(x4)
        x4_depth = self.rgb.forward_featuresFour(x4_depth)
        x4_1 = self.dea4(x4, x4_depth)
        x4_2 = self.eca(x4_1)
...
if __name__ == '__main__':
    ...
    model = DCNet_S().to(torch.device("cuda:0"))
    out = model(img, depth)
